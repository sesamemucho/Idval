...
#     # It's important to deal with duplicate keys in the selectors
#     foreach my $sel (@{$fname})
#     {
#         my ($key, $op, $value) = @{$sel};
#         push(@{$self->{SELECTORS}->{$key}}, [$op, $value]);
#     }
# }
...
#     foreach my $key (keys %{$self->{SELECTORS}})
#     {
#         if (! exists($items->{$key}))
#         {
#             $retval = 0;
...
#         $dupval = 0;
#         foreach my $sels (@{$self->{SELECTORS}->{$key}})
#         {
#             my ($op, $value) = @{$sels};
#             my $cmpfunc = $compare_function{$op}->{FUNC}->{STR};
#             #print STDERR "Comparing \"$value\" \"$op\" \"$items->{$key}\" resulted in ", 
#             #    &$cmpfunc($items->{$key}, $value) ? "True\n" : "False\n";
#             $dupval ||= &$cmpfunc($items->{$key}, $value);
#         }
