...
sub read_string {
	...
	##RSF @lines and $lines[0] =~ /^\%YAML[: ][\d\.]+.*$/ and shift @lines;
        $self->[0] = scalar @lines;

	# A nibbling parser
	while ( @lines ) {

                next if $lines[0] =~ /^\%YAML[: ][\d\.]+.*$/;
		...
		if ( ! @lines or $lines[0] =~ /^(?:---|\.\.\.)/ ) {
			...
			$self->die_with_bad_line(\@lines, "YAML::Tiny does not support the line '$lines[0]'");
		}
	}
...
sub _read_scalar {
	...
	# Special cases
	$self->die_with_bad_line($lines, "Unsupported YAML feature") if $string =~ /^['"!&]/;
	return {} if $string eq '{}';
	return [] if $string eq '[]';
	...
	# Error
	$self->die_with_bad_line($lines, "Multi-line scalar content missing") unless @$lines;

	# Check the indent depth
	$lines->[0] =~ /^(\s*)/;
	$indent->[-1] = length("$1");
	if ( defined $indent->[-2] and $indent->[-1] <= $indent->[-2] ) {
		$self->die_with_bad_line($lines, "Illegal line indenting");
	}
...
sub _read_array {
	...
	while ( @$lines ) {
		...
		if ( length($1) < $indent->[-1] ) {
			...
			$self->die_with_bad_line($lines, "Hash line over-indented");
		}
...
sub _read_hash {
	...
	while ( @$lines ) {
		...
		# Get the key
		unless ( $lines->[0] =~ s/^\s*([^\'\" ][^\n]*?)\s*:(\s+|$)// ) {
			$self->die_with_bad_line($lines, "Unsupported YAML feature") if $lines->[0] =~ /^\s*[?'"]/;
			$self->die_with_bad_line($lines, "Bad or unsupported hash line");
		}
		my $key = $1;
...
sub die_with_bad_line {
    my $self = shift;
    my $lines = shift;
    my @errinfo = @_;

    my $badline_number = $self->[0] - scalar(@$lines);
    my $badline = @$lines ? $lines->[0] : '';

    die "At line $badline_number: ", @errinfo, " \"$badline\"";
}        
