package Idval::Plugin::Command::Foo;
...
#     {
#         #return $datastore if $first > 5;
#         my $tag_record = $datastore->{RECORDS}->{$key};

#         print "foo: ", Dumper($tag_record);
#         return $datastore;
...
#         my $select = $config->merge_blocks($tag_record);
# #         if ($first > 10)
# #         {
...
#         if ($select)
#         {
#             print "Got merge block for ", $tag_record->get_name(), " ($numrecs)\n";
#             #print "mb: ", Dumper($select) if $first <= 2;
#             print "mb: ", Dumper($select);
#             $numrecs++;
#         }
#     }
