...
# }
# EOF
#     my $obj = Idval::NConfig->new('');

#     my $subr = $obj->config_to_subr("{\nfoo = 1\n}\n");
...
#   {
#     if (
#         exists($selects->{gubber}) and
#         Idval::Select::cmp_eq_num($selects->{gubber}, q{33})
#        )
#     {
#       $vars{foo} = q{1};
#     }
...
#     if (
#         exists($selects->{gubber}) and
#         (Idval::Select::cmp_eq_num($selects->{gubber}, q{33}) or
#          Idval::Select::cmp_eq_num($selects->{gubber}, q{34}))
#        )
#     {
#       $vars{foo} = q{1};
#     }
...
#   {
#     if (
#         exists($selects->{garfle}) and
#         Idval::Select::cmp_lt_num($selects->{garfle}, q{2}) and
#         exists($selects->{gubber}) and
#         (Idval::Select::cmp_eq_num($selects->{gubber}, q{33}) or
#          Idval::Select::cmp_eq_num($selects->{gubber}, q{34}))
#        )
...
#     {
#       $vars{foo} = q{1};
#       exists($vars{foo}) ? [$vars{foo}, q{3}] : q{3};
#     }
#   }
#   return \%vars;
# }
...
#     $vars = $obj->merge_blocks({gubber => 35});
#     is_deeply($vars, {});
...
#     my $obj = Idval::NConfig->new('/testdir/gt1.txt');
#     # Since the block has no selects, we can pass in any selectors and it should succeed
...
#     my $cfg_dbg = 1;
#     #my $cfg_dbg = 0;
#     my $obj = Idval::NConfig->new('/testdir/gt1.txt', $cfg_dbg);
...
# # #     my $reader = Idval::NConfig::BlockReader->new(1);
...
# # #     #my $self = shift;
# # #     Idval::FileString::idv_add_file('/testdir/gt1.txt', "\ngubber = pachoo\r\n\rhubber=4\n\n");
# # #     my $obj = Idval::NConfig->new('/testdir/gt1.txt', Idval::NConfig::STRICT_MATCH);
...
# # #     my $obj = Idval::NConfig->new('/testdir/gt1.txt', Idval::NConfig::LOOSE_MATCH);
...
EOF
    ...
    *{'Idval::ValidateFuncs::TestSub'} = sub {
    ...
    Idval::FileString::idv_add_file('/testdir/gt1.txt', $cfg_file);
    #$Idval::NConfig::USE_LOGGER = 0;
    my $obj = Idval::NConfig->new('/testdir/gt1.txt');

    #print "new config: ", Dumper($obj);
    #$Idval::NConfig::DEBUG = 1;
    #$obj->{DEBUG} = 1;
    my $val = $obj->get_value('myval', {'AAA' => 'aaa aaa'});
    is($val, 3);
    #$Idval::NConfig::DEBUG = 0;
    #$Idval::NConfig::USE_LOGGER = 1;
}

# sub two_selectors_and_together : Test(4)
# {
# my $cfg_file =<<EOF;
# blah_blah = 1
# {
...
#     my $obj = Idval::NConfig->new($cfg_file);

#     is($obj->get_single_value('blah_blah', {foo => 1}), 1);
#     is($obj->get_single_value('blah_blah', {foo => 3}), 3);
#     is($obj->get_single_value('blah_blah', {foo => 5}), 3);
#     is($obj->get_single_value('blah_blah', {foo => 6}), 1);
# }
...
# #     memoize('Idval::NConfig::_merge_blocks', INSTALL => 'Idval::NConfig::fast_merge_blocks',
# #             SCALAR_CACHE => [HASH => \%memohash]);

# # #     memoize('Idval::NConfig::_merge_blocks', INSTALL => 'Idval::NConfig::fast_merge_blocks_1',
# # #             NORMALIZER => 'normalize',
# # #             SCALAR_CACHE => [HASH => \%memohash1]);

# #     my $save = *Idval::NConfig::get_single_value;
# #     *Idval::NConfig::get_single_value = *fast_get_single_value;

# #     my $bm1 = timethis(10000, sub {$obj->get_single_value("gubber", $sel)});
# #     *Idval::NConfig::get_single_value = $save;
...
# #     my $save = *Idval::NConfig::get_single_value;
# #     *Idval::NConfig::get_single_value = *fast_get_single_value1;
...
# #     print STDERR "\nmemo took ", timestr($bm1), "\n";
# #     print STDERR "non-memo took ", timestr($bm2), "\n";
# #     print STDERR "number of keys: ", scalar(keys %Idval::NConfig::memohash), "\n";
# # }
...
# #     my ($k, $v) = each(%Idval::NConfig::memohash);
# #     print STDERR "\nmemo took ", timestr($bm1), "\n";
# #     print STDERR "number of keys: ", scalar(keys %Idval::NConfig::memohash), "\n";
# #     my $i = 0;
# #     foreach my $key (keys %Idval::NConfig::memohash)
# #     {
# #         print STDERR "sample hashval: \"$key\" => \"$Idval::NConfig::memohash{$key}\"\n";
